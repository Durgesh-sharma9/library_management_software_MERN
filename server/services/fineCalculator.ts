import { IFineRule } from '../models/LibrarySetting.js';
 ...
 * - Total fine = ₹12 + ₹15 = ₹27
 */
export function calculateFineBreakdown(
  dueDateInput: Date | string,
  targetDateInput: Date | string = new Date(),
  setting?: { finePerDay?: number; fineRules?: IFineRule[] } | null
): CalculatedFineResult {
  const due = new Date(dueDateInput);
  const target = new Date(targetDateInput);

  // Normalize to calendar dates (midnight) to ensure clean whole-day calculations
  ...
  // If target date is on or before due date, no fine
  ...
  const defaultRate = setting?.finePerDay !== undefined && setting.finePerDay !== null
    ? Math.max(0, Number(setting.finePerDay))
    : 2;

  // Prepare normalized rules sorted ascending by date
  let sortedRules: Array<{ dateMidnight: Date; rate: number; note?: string }> = [];

  if (setting?.fineRules && Array.isArray(setting.fineRules) && setting.fineRules.length > 0) {
    sortedRules = setting.fineRules
  ...
  // If no valid rules found in array, create a single baseline rule with defaultRate
  ...
  // Day-by-day progression: overdue begins the day after dueDate
  ...
  // Aggregate contiguous days with identical rates into clean slabs
